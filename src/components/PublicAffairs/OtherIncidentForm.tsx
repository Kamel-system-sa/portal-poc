...
import type { OtherIncident, IncidentType } from '../../data/mockPublicAffairs';

const { TextArea } = Input;

interface OtherIncidentFormProps {
  open: boolean;
  onCancel: () => void;
  onSubmit: (data: Omit<OtherIncident, 'id' | 'createdAt' | 'completed' | 'completedAt' | 'resolved'>) => void;
}

export const OtherIncidentForm: React.FC<OtherIncidentFormProps> = ({
  open,
  onCancel,
  onSubmit
}) => {
  ...
  const [formData, setFormData] = useState({
    ...
    passportNumber: '',
    nationality: '',
    incidentType: 'missing' as IncidentType,
    organizerNumber: '',
    nusukCaseNumber: '',
    description: ''
  });
  ...
  const incidentTypeOptions = [
    ...
    { value: 'lost_passport', label: t('lostPassport') }
  ];
  ...
  const handleSubmit = () => {
    ...
    if (!formData.organizerNumber.trim()) {
      newErrors.organizerNumber = t('required');
    }
    ...
    >
      <div className="p-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">
          {t('registerOtherIncident')}
        </h2>

        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          {/* Horizontal Layout: Personal Information (Left) and Incident Information (Right) */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <label className="block">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="block text-sm font-semibold text-gray-700">{t('name')} <span className="text-red-500">*</span></span>
                  </div>
                  <input
                    type="text"
            ...
            {/* Right Side: Incident Information */}
            <section className="bg-white rounded-2xl shadow-lg shadow-gray-200/50 p-6 border border-gray-100">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-1 h-6 bg-gradient-to-b from-mainColor to-primary rounded-full"></div>
              ...
              <div className="space-y-4">
                <label className="block">
                  <div className="flex items-center gap-2 mb-2">
                    <WarningOutlined className="text-mainColor text-base" />
                    <span className="block text-sm font-semibold text-gray-700">{t('incidentType')} <span className="text-red-500">*</span></span>
                  </div>
                  <Select
                ...
                <label className="block">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="block text-sm font-semibold text-gray-700">{t('organizerNumber')} <span className="text-red-500">*</span></span>
                  </div>
                  <input
                    type="text"
                    className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-mainColor/20 focus:border-mainColor transition-all duration-200 bg-white shadow-sm hover:shadow-md text-gray-700 ${
                      errors.organizerNumber ? 'border-red-500' : 'border-gray-200'
                    }`}
                    placeholder={t('enterOrganizerNumber')}
                    value={formData.organizerNumber}
                    onChange={(e) => updateField('organizerNumber', e.target.value)}
                    required
                  />
                  {errors.organizerNumber && <p className="text-red-500 text-xs mt-1">{errors.organizerNumber}</p>}
                </label>

                <label className="block">
                  <div className="flex items-center gap-2 mb-2">
                    <FileTextOutlined className="text-mainColor text-base" />
                    <span className="block text-sm font-semibold text-gray-700">{t('description')}</span>
                  </div>
                  <TextArea
