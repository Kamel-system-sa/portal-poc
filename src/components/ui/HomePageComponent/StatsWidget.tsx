        ...
        return (
          <Col xs={12} sm={12} md={6} key={index}>
            ...
            >
              ...
              <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-white/40 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              
              <div className="relative z-10">
                <div className={`text-2xl mb-2 ${stat.color} flex justify-center group-hover:scale-110 transition-transform duration-300`}>
                  {stat.icon}
                </div>
                <div className="text-center">
                  <Statistic
                    title={<span className="text-gray-600 text-xs font-medium">{stat.title}</span>}
                    value={stat.value}
                    valueStyle={{ 
                      fontSize: '22px', 
                      fontWeight: 700, 
