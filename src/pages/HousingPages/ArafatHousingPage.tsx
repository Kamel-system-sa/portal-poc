...
import { mockArafatTents } from '../../data/mockHousing';
...
const ArafatHousingPage: React.FC = () => {
  ...
  const filteredTents = useMemo(() => {
    return mockArafatTents.filter((tent: Tent) => {
      // Search filter
  ...
  const sections = useMemo(() => {
    const uniqueSections = new Set(mockArafatTents.map(t => t.section).filter(Boolean));
